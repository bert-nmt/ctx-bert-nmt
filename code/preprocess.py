...
def main(args):
    ...
    def make_binary_dataset(vocab, input_prefix, output_prefix, lang, num_workers):
        ...
        output_prefix += '.bert.doc' if isinstance(vocab, BertTokenizer) else ''  # for convenience
        input_prefix += ('.bert.' + args.bert_input) if isinstance(vocab, BertTokenizer) else ''
        n_seq_tok = [0, 0]
        ntrunc = None
        replaced = Counter()
        ...
        def merge_result(worker_result):
            ...
            nonlocal ntrunc
            if worker_result["ntrunc"] is not None:
                ntrunc = (0 if ntrunc is None else ntrunc) + worker_result["ntrunc"]
        ...
        if ntrunc is not None:
            print("| ======== Important! {:.3}% sentences truncated ({:d} / {:d})".format(ntrunc / n_seq_tok[0], ntrunc, n_seq_tok[0]))
...
def cli_main():
    ...
    parser.add_argument('--bert-input', default='doc', choices=['doc', 'sent'])
