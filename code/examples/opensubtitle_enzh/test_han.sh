...
cktpath=$1
if [ ! -f $cktpath ]; then
  echo >&2 "Checkpoint $cktpath doesn't exist!"
  exit
fi
...
echo >&2 "Testing $srclng to $tgtlng, subset: $subset, output to $tgtlog"
echo >&2 "Using data $DATA_PATH"
...
python $BASHDIR/generate.py $DATA_PATH --gen-subset $subset --path $cktpath --task translation_han_v2 \
