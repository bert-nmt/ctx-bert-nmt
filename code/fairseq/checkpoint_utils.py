...
def load_checkpoint(args, trainer):
    ...
    # pdb.set_trace()
    if args.max_update == 0 and args.incremental_update > 0 and args.incremental_update_from is not None:
        assert os.path.exists(args.incremental_update_from)
        state = load_checkpoint_to_cpu(args.incremental_update_from)
        current_steps = state['optimizer_history'][-1]['num_updates']
        print("| Load state from %s, which has been trained for %d steps" % (args.incremental_update_from, current_steps))
        args.max_update = current_steps + args.incremental_update
        print("| Total training step: %d" % args.max_update)
    else:
        current_steps = 0
