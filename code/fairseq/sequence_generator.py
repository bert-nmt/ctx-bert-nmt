...
class SequenceGenerator(object):
    ...
    def generate(
        ...
        **kwargs
        ...
        # def gettype(bert_input, berttokenizer):
        #     x = torch.cat((bert_input[:, 0].unsqueeze(-1), bert_input[:, :-1]), dim=1)
        #     y = x.eq(berttokenizer.sep()).type(x.type())
        #     y = y.cumsum(1)
        #     return y
        #     token_type_ids = model.models[0].gettype(bertinput, model.models[0].berttokenizer)
        #     bert_encoder_outs, _ = model.models[0].bert_encoder(bertinput,token_type_ids=token_type_ids, output_all_encoded_layers=True, attention_mask= 1. - bert_encoder_padding_mask)
        #     bert_encoder_outs = bert_encoder_outs[self.bert_output_layer]
        ...
        #     bert_encoder_outs = bert_encoder_outs.permute(1,0,2).contiguous()
            # bert_encoder_outs = F.linear(bert_encoder_outs, model.models[0].trans_weight, model.models[0].trans_bias)
        #     bert_encoder_outs = [{
        #         'bert_encoder_out': bert_encoder_outs,
        #         'bert_encoder_padding_mask': bert_encoder_padding_mask,
        #     }]
        #     encoder_input['bert_encoder_out'] = bert_encoder_outs[0]
        if model.models[0].__class__.__name__ in ['TransformerS2Model', "TransformerHANModel", ]:
            bert_keys = inspect.getargspec(model.models[0].get_bert_embeddings).args[1:]
            bert_vals = []
            for k in bert_keys:
                if k in sample:
                    bert_vals.append(sample[k])
                else:
                    bert_vals.append(encoder_input.pop(k))
            bert_encoder_outs = model.models[0].get_bert_embeddings(*bert_vals)
            bert_encoder_outs = [bert_encoder_outs, ]
            encoder_input['bert_encoder_out'] = bert_encoder_outs[0]
        elif 'bert_encoder_input' in encoder_input:
            # pdb.set_trace()
            bert_encoder_outs = model.models[0].forward_bert(**encoder_input.pop('bert_encoder_input'))
            encoder_input['bert_encoder_out'] = bert_encoder_outs
            bert_encoder_outs = [bert_encoder_outs, ]
        ...
        encoder_outs, bert_encoder_outs = model.reorder_encoder_out(encoder_outs, bert_encoder_outs, new_order)
        ...
        for step in range(max_len + 1):  # one extra step for EOS marker
            ...
            if reorder_state is not None:
                ...
                encoder_outs, bert_encoder_outs = model.reorder_encoder_out(encoder_outs, bert_encoder_outs, reorder_state)

            lprobs, avg_attn_scores = model.forward_decoder(
                tokens[:, :step + 1], encoder_outs, bert_encoder_outs=bert_encoder_outs, temperature=self.temperature,
            )
...
class EnsembleModel(torch.nn.Module):
    ...
    @torch.no_grad()
    def forward_decoder(self, tokens, encoder_outs, bert_encoder_outs, temperature=1.):
        if len(self.models) == 1:
            ...
            return self._decode_one(
                ...
                bert_encoder_out=bert_encoder_outs[0],
                log_probs=True,
    ...
    def _decode_one(
        ...
        bert_encoder_out=None, temperature=1.,
    ):
        if self.incremental_states is not None:
            decoder_out = list(model.decoder(tokens, encoder_out, bert_encoder_out=bert_encoder_out, incremental_state=self.incremental_states[model]))
        else:
            decoder_out = list(model.decoder(tokens, encoder_out, bert_encoder_out=bert_encoder_out))
    ...
    def reorder_encoder_out(self, encoder_outs, bert_encoder_outs, new_order):
        if not self.has_encoder():
            return
        model = self.models[0]
        encoder_outs, bert_encoder_outs = model.encoder.reorder_encoder_out(encoder_outs[0], bert_encoder_outs[0], new_order)
        return [encoder_outs], [bert_encoder_outs]
