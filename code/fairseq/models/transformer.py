...
class TransformerS2Model(FairseqEncoderDecoderModel):
    ...
    def add_args(parser):
        ...
        parser.add_argument('--adaptive-softmax-dropout', type=float, metavar='D',
                            help='sets adaptive softmax dropout for the tail projections')
        # fmt: on
        # BERT seg id embeddings args
        parser.add_argument('--seg-all-zero', default=False, action='store_true')
        parser.add_argument('--seg-overflow-keep', default='max', choices=['max', 'min'],
                            help="If segment embeddings have over 2 segments, " + \
                                 "keep which: keep max as 1, or keep min as 0")
        parser.add_argument('--learnable-bert-ratio', default=None, choices=["sigmoid", ])
        parser.add_argument('--use-bert-segments', default=None, type=int, nargs='+')
        # ablation
        parser.add_argument('--word-embeddings-only', default=False, action='store_true')
    ...
    def gettype(self, bert_input, berttokenizer):
        ...
        y = y.cumsum(1)
        # pdb.set_trace()
        # print("======== DEBUG: seg_all_zero %r seg_overflow_keep %r" % (self.args.seg_all_zero, self.args.seg_overflow_keep))
        if not hasattr(self.args, 'seg_all_zero') or not hasattr(self.args, 'seg_overflow_keep'):
            print("======== DEBUG: old version, lack para!")
        elif self.args.seg_all_zero:
            y[:] = 0
        elif y.max() > 1:
            if self.args.seg_overflow_keep == 'max':
                y -= y.max() - 1
                y[y < 0] = 0
            else:
                y[y > 1] = 1
    ...
    def get_bert_embeddings(self, bert_input):
        bert_encoder_padding_mask = bert_input.eq(self.berttokenizer.pad())

        token_type_ids = self.gettype(bert_input, self.berttokenizer)
        if getattr(self.args, "word_embeddings_only", False):
            # pdb.set_trace()
            words_embeddings = self.bert_encoder.embeddings.word_embeddings(bert_input)
            token_type_embeddings = self.bert_encoder.embeddings.token_type_embeddings(token_type_ids)
            bert_encoder_out = words_embeddings + token_type_embeddings
        else:
        ...
        # mask unused segments
        if getattr(self.args, "use_bert_segments", None) is not None:
            # pdb.set_trace()
            token_type_ids_ = torch.cat((bert_input[:, 0].unsqueeze(-1), bert_input[:, :-1]), dim=1)
            token_type_ids_ = token_type_ids_.eq(self.berttokenizer.sep()).type(token_type_ids_.type())
            token_type_ids_ = token_type_ids_.cumsum(1)
            mask_ = torch.ones_like(bert_encoder_padding_mask)
            for s in self.args.use_bert_segments:
                mask_ *= (token_type_ids_ != s)
            bert_encoder_padding_mask |= mask_
    ...
    def forward(self, src_tokens, src_lengths, prev_output_tokens, bert_input, **kwargs):
        ...
        """
        bert_encoder_out = self.get_bert_embeddings(bert_input)
    ...
    def update_encoder_bert_dropout_ratio(self, r):
        # pdb.set_trace()
        for l in self.encoder.layers:
            l.encoder_bert_dropout_ratio = r
...
class TransformerS2EncoderLayer(nn.Module):
    ...
    *args.encoder_normalize_before* to ``True``.
    ...
    def __init__(self, args, bert_gate=True):
        ...
        self.learnable_bert_ratio = getattr(args, "learnable_bert_ratio", None)
        if self.learnable_bert_ratio is not None:
            if self.learnable_bert_ratio == "sigmoid":
                self.bert_ratio_module = nn.Linear(self.embed_dim * 2, 2)
            else:
                raise ValueError("Invalid learnable ratio %s" % self.learnable_bert_ratio)
    ...
    def get_ratio(self, x1, x2):
        if self.learnable_bert_ratio is not None:
            if self.learnable_bert_ratio == "sigmoid":
                # pdb.set_trace()
                ratio = self.bert_ratio_module(torch.cat([x1, x2], dim=-1))
                ratio = torch.sigmoid(ratio)
                return [ratio[:, :, :1], ratio[:, :, 1:]]
            else:
                raise ValueError("Invalid learnable ratio %s" % self.learnable_bert_ratio)
        elif self.discard_bert:
