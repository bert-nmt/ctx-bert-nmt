...
def batch_by_size_with_bert(
    indices, num_tokens_fn, num_bert_tokens_fn,
    max_tokens=None, max_bert_tokens=None, max_sentences=None,
    required_batch_size_multiple=1,
):
    """
    Yield mini-batches of indices bucketed by size. Batches may contain
    ...
    # pdb.set_trace()
    max_tokens = max_tokens if max_tokens is not None else float('Inf')
    max_bert_tokens = max_bert_tokens if max_bert_tokens is not None else float('Inf')
    max_sentences = max_sentences if max_sentences is not None else float('Inf')
    bsz_mult = required_batch_size_multiple

    batch = []

    def is_batch_full(num_tokens, num_bert_tokens):
        if len(batch) == 0:
            return False
        if len(batch) == max_sentences:
            return True
        if num_tokens > max_tokens:
            return True
        if num_bert_tokens > max_bert_tokens:
            return True
        return False
    ...
    for idx in indices:
        ...
        num_tokens = (len(batch) + 1) * sample_len
        # bert tokens
        bert_lens.append(num_bert_tokens_fn(idx))
        bert_len = max(bert_len, bert_lens[-1])
        assert bert_len <= max_bert_tokens, (
            "sentence at index {} of size {} exceeds max_tokens "
            "limit of {}!".format(idx, sample_len, max_tokens)
        )
        num_bert_tokens = (len(batch) + 1) * bert_len
        # batch
        if is_batch_full(num_tokens, num_bert_tokens):
            mod_len = max(
                bsz_mult * (len(batch) // bsz_mult),
                len(batch) % bsz_mult,
            )
